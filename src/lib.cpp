...
void internal_recursive_len(LookaheadParser* r, int& len) {
    if (r->PeekType() == kNullType) {
        // printf("%d: null\n", len);
        r->GetNull();
        len++;
    } else if (r->PeekType() == kTrueType || r->PeekType() == kFalseType) {
        r->GetBool();
        // printf("%d: bool(%d)\n", len, val);
        len++;
    } else if (r->PeekType() == kNumberType) {
        r->GetDouble();
        // printf("%d: double(%f)\n", len, val);
        len++;
...
int recursive_len(char* filename) {
    using namespace std;

    // printf("filename(%s)", filename);
    char* json = read_file_to_string("large-file.json");
...
int multiple_recursive_len(char* filename) {
    int len = 0;
    for (int i = 0; i <= 1000000; i++) {
        len = recursive_len(filename);
    }
    return len;
}
...
extern "C" {
    ...
    int strlen_c(char* string) {
        int length = -1;
        printf("message from strlen_c hello\n");
        while (string[++length] != '\0') {
            printf("%d: %c", length, string[length]);
        }
        return length;
    }

    int recursive_len_c(char* filename) {
        return recursive_len(filename);
    }

    int multiple_len_c(char* filename) {
        return multiple_recursive_len(filename);
    }
}